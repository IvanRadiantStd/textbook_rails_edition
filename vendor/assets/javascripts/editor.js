...
var sendRequest = function(tex, my_counter, onthefly){
  ...
  $.post("http://latexml.mathweb.org/convert", { "tex": tex, "preamble" :
...
if (tex_requested) {
  ...
  example_option = $('#example_select').find('option[value="'+example_requested+'"]').first()
  if (example_option.length == 0) {
    example_option = $('#example_select').find('option:contains("'+example_requested+'")').first(); }
  if (example_option.length > 0) {
  ...
  editor.setValue($(".text-text").text()); }
editor.clearSelection();
